	...
	(() => {
					...
					return res.status === 200
						...
						.then(data => {
...

							{
								...
								audio_reference: (data.screen_template_map[data.learnables[key].learnable_id].presentation[0].audio.value[0]==null)?null:data.screen_template_map[data.learnables[key].learnable_id].presentation[0].audio.value[0].normal
		...
		.then(words => {
			...
			return words.map(word => word.translation + '\t' + word.original + '\t[memrise_' + (word.audio_reference==null?'':word.audio_reference.slice(word.audio_reference.indexOf('.com')+5).replace(/\//g,'_')) + ']\n').join('') //for 4 data things
